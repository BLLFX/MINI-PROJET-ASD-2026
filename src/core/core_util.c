...
int core_supprimer_enreg(const char *nom_fichier, size_t taille_enreg,
    ...
    /* Ecriture atomique : fichier temporaire puis rename(). Un crash
     * pendant l'ecriture laisse le fichier original intact. */
    {
        char chemin_tmp[PATH_MAX];
        int m = snprintf(chemin_tmp, sizeof(chemin_tmp), "%s/%s.tmp",
                         g_dossier, nom_fichier);
        char chemin_fin[PATH_MAX];
        int m2;
        if (m < 0 || (size_t)m >= sizeof(chemin_tmp)) {
            free(tab);
            return EL_ERR_FICHIER;
        }
        f = fopen(chemin_tmp, "wb");
        if (f == NULL) {
            free(tab);
            return EL_ERR_FICHIER;
        }
        ...
        m2 = snprintf(chemin_fin, sizeof(chemin_fin), "%s/%s",
                      g_dossier, nom_fichier);
        if (m2 < 0 || (size_t)m2 >= sizeof(chemin_fin) ||
            rename(chemin_tmp, chemin_fin) != 0) {
            remove(chemin_tmp);
            free(tab);
            return EL_ERR_FICHIER;
        }
...
int core_remplacer_enreg(const char *nom_fichier, size_t taille_enreg,
    ...
    while (fread(temp, taille_enreg, 1, f) == 1) {
        if (correspond(temp, cle)) {
            if (fseek(f, -((long)taille_enreg), SEEK_CUR) != 0 ||
                fwrite(nouvel, taille_enreg, 1, f) != 1 ||
                fflush(f) != 0) {
                free(temp);
                fclose(f);
                return EL_ERR_FICHIER;
