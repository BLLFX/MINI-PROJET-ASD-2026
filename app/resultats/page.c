...
static void res_refresh(ui_widget_t *w)
{
        ...
        /* --- Votes blancs / non blancs par departement ---
         * Agregation des bureaux de vote par departement. */
        ui_widget_empty(_that->refs.departements);
        {
                typedef struct {
                        char departement[30];
                        int blancs;
                ...
                dept_stat_t *deps = calloc(
                        (size_t)(res.nb_bureaux > 0 ? res.nb_bureaux : 1),
                        sizeof(dept_stat_t));
                int n_deps = 0;
                if (deps != NULL) {
                        for (i = 0; i < res.nb_bureaux; i++) {
                                ...
                                const char *dep =
                                        rb->departement[0]
                                                ...
                                                : "Inconnu";
                                for (j = 0; j < n_deps; j++) {
                                        if (strcmp(deps[j].departement,
                                                   dep) == 0) {
                                                break;
                                        }
                                }
                                if (j == n_deps) {
                                        snprintf(deps[j].departement,
                                                 sizeof(deps[j].departement),
                                                 "%s", dep);
                                        n_deps++;
                                }
                                deps[j].blancs += rb->votes_blancs;
                                deps[j].non_blancs += rb->votes_non_blancs;
                        }
                        if (n_deps == 0) {
                                ui_widget_append(_that->refs.departements,
                                        backend_text(
                                                "text-body-sm text-secondary py-2",
                                                "Aucun département pour le moment."));
                        }
                        for (i = 0; i < n_deps; i++) {
                                int total_dept =
                                        deps[i].blancs + deps[i].non_blancs;
                                float pct = total_dept > 0
                                                ? 100.0f * deps[i].non_blancs /
                                                          total_dept
                                                : 0.0f;
                                ui_widget_t *ligne =
                                        res_ligne_stat(deps[i].departement,
                                                       deps[i].non_blancs,
                                                       pct);
                                snprintf(tampon, sizeof(tampon),
                                         "%d blancs", deps[i].blancs);
                                ui_widget_append(ligne,
                                        backend_cell(
                                                "w-20 text-right", tampon,
